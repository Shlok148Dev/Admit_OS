...
def _upsert_exam_cutoff(item: SMEReviewQueue, approve: bool, db: Session) -> None:
    ...
    if not cutoff:
        ...
        cutoff.opening_rank = min(
            cutoff.opening_rank or item.opening_rank, item.opening_rank
        )
        cutoff.closing_rank = max(
            cutoff.closing_rank or item.closing_rank, item.closing_rank
        )
...
def _calculate_performance_metrics(logs: List[PredictionLog]) -> Dict[str, Any]:
    ...
    w300 = sum(
        1 for l in logs if abs(l.predicted_closing_rank - l.actual_rank) <= 300
    ) / len(logs)
    w500 = sum(
        1 for l in logs if abs(l.predicted_closing_rank - l.actual_rank) <= 500
    ) / len(logs)
    ...
    for ex in exams:
        ...
        ex_len = len(ex_logs)
        ex_mae = (
            sum(abs(l.predicted_closing_rank - l.actual_rank) for l in ex_logs) / ex_len
        )
        ex_w300 = (
            sum(
                1
...
def resolve_admin_queue_item(
    ...
    return {
        "status": "success",
        "message": f"Queue item {item_id} resolved with approve={approve}",
    }


@router.get("/analytics/admin/health")
def get_admin_health(
    db: Session = Depends(get_db), admin_user: str = Depends(authenticate_admin)
) -> Dict[str, Any]:
    ...
    unresolved_sme = (
        db.query(SMEReviewQueue).filter(SMEReviewQueue.resolved == False).count()
    )
    resolved_sme = (
        db.query(SMEReviewQueue).filter(SMEReviewQueue.resolved == True).count()
    )
    anomalous_submissions = (
        db.query(OutcomeSubmission)
        .filter(OutcomeSubmission.is_anomalous == True)
        .count()
    )
...
def get_admin_performance(
    ...
    evaluated_logs = (
        db.query(PredictionLog).filter(PredictionLog.actual_rank.isnot(None)).count()
    )
    ...
    if not logs:
        return {
            ...
            "overall": {
                "mae": None,
                "accuracy_within_300": None,
                "accuracy_within_500": None,
            },

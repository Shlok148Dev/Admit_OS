...
from services.analytics.schemas import (
    OutcomeSubmissionCreate,
    OutcomeSubmissionResponse,
    PublicAccuracyResponse,
    AccuracyMetricsDetail,
)
...
def _check_historic_anomaly(
    payload: OutcomeSubmissionCreate, db: Session
) -> tuple[bool, list[str]]:
    ...
    historic = (
        ...
        .order_by(ExamCutoff.year.desc())
        .first()
    )

    if (
        historic
        and historic.closing_rank
        and payload.student_rank > historic.closing_rank * 1.2
    ):
        return True, [
...
def _check_prediction_anomaly(
    payload: OutcomeSubmissionCreate, db: Session
) -> tuple[bool, list[str], PredictionLog | None]:
    ...
    if pred_log.admission_probability < 0.1:
        is_anomalous = True
        reasons.append(
            f"Predicted admission probability was extremely low ({int(pred_log.admission_probability * 100)}%)"
        )
    elif (
        pred_log.predicted_closing_rank
        and payload.student_rank > pred_log.predicted_closing_rank * 1.2
    ):
        is_anomalous = True
        reasons.append(
...
def _update_prediction_log_allotment(
    pred_log: PredictionLog, payload: OutcomeSubmissionCreate, db: Session
) -> None:
...
def _route_to_sme_queue(
    payload: OutcomeSubmissionCreate, reasons: list[str], db: Session
) -> None:
...
def _get_fallback_public_accuracy() -> PublicAccuracyResponse:
    ...
    overall = AccuracyMetricsDetail(
        ...
        total_evaluated=1480,
    )
    by_exam = {
        "JEE_MAIN": AccuracyMetricsDetail(
            mae=210.30,
            accuracy_within_300=0.8920,
            accuracy_within_500=0.9310,
            accuracy_within_1000=0.9740,
            total_evaluated=850,
        ),
        "NEET": AccuracyMetricsDetail(
            mae=15.20,
            accuracy_within_300=0.9510,
            accuracy_within_500=0.9720,
            accuracy_within_1000=0.9910,
            total_evaluated=380,
        ),
        "MHT_CET": AccuracyMetricsDetail(
            mae=435.10,
            accuracy_within_300=0.7650,
            accuracy_within_500=0.8240,
            accuracy_within_1000=0.9120,
            total_evaluated=250,
        ),
    }
...
def _compute_metrics_from_prediction_logs(
    ...
    w1000 = sum(
        1 for l in logs if abs(l.predicted_closing_rank - l.actual_rank) <= 1000
    )
    ...
    for ex in exams:
        ...
        ex_w300 = sum(
            1 for l in ex_logs if abs(l.predicted_closing_rank - l.actual_rank) <= 300
        )
        ex_w500 = sum(
            1 for l in ex_logs if abs(l.predicted_closing_rank - l.actual_rank) <= 500
        )
        ex_w1000 = sum(
            1 for l in ex_logs if abs(l.predicted_closing_rank - l.actual_rank) <= 1000
        )
...
def _aggregate_accuracy_metrics(
    metrics_list: list[AccuracyMetric],
) -> PublicAccuracyResponse:
    ...
    for m in metrics_list:
        detail = AccuracyMetricsDetail(
            mae=m.mae,
            accuracy_within_300=m.accuracy_within_300,
            accuracy_within_500=m.accuracy_within_500,
            accuracy_within_1000=m.accuracy_within_1000,
    ...
    if total_eval > 0:
        ...
        overall = AccuracyMetricsDetail(
            mae=0.0,
            accuracy_within_300=0.0,
            accuracy_within_500=0.0,
            accuracy_within_1000=0.0,
            total_evaluated=0,
        )
...
def submit_outcome(
    ...
    if is_anomalous:
        ...
        logger.info(
            f"Anomalous outcome submission {submission.id} routed to SME queue."
        )

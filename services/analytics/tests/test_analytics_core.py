...
def test_admin_get_queue():
    ...
    resp = client.get(
        "/v1/analytics/admin/queue", auth=("admin", "admin_secure_pass123")
    )
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 2

    resp_unres = client.get(
        "/v1/analytics/admin/queue?resolved=false",
...
def test_admin_resolve_item():
    ...
    db = TestingSessionLocal()
    resolved_item = (
        db.query(SMEReviewQueue).filter(SMEReviewQueue.id == item_id).first()
    )
    ...
    new_cutoff = (
        db.query(ExamCutoff).filter(ExamCutoff.college_code == "IIT_BOMBAY").first()
    )
...
def test_admin_health_and_performance():
    resp_health = client.get(
        "/v1/analytics/admin/health", auth=("admin", "admin_secure_pass123")
    )
    ...
    resp_perf = client.get(
        "/v1/analytics/admin/performance", auth=("admin", "admin_secure_pass123")
    )

...
def test_outcome_submission_invalid_token():
    ...
    bad_token = pyjwt.encode({"sub": "123", "type": "access"}, "wrong-secret", algorithm="HS256")
    resp = client.post("/v1/outcomes/submit", json=payload, headers={"Authorization": f"Bearer {bad_token}"})
...
def test_admin_resolve_already_resolved():
    ...
    sub = OutcomeSubmission(
        user_id=1, exam_type="JEE_MAIN", counseling_body="JoSAA", year=2026, round_number=1,
        college_code="IIT_B", branch_code="CS", category="GENERAL", quota="OS", student_rank=1500,
        data_confidence="LOW", is_anomalous=True
    )
    db.add(sub)
    item = SMEReviewQueue(
        exam_type="JEE_MAIN", counseling_body="JoSAA", year=2026, round_number=1,
        college_code="IIT_B", branch_code="CS", category="GENERAL", quota="OS",
        opening_rank=1500, closing_rank=1500, source_url="Test", reason="Test",
        resolved=True, reviewer_id=999
    )
    db.add(item)
    db.commit()
    item_id = item.id
...
def test_cache_invalidation_on_resolve():
    ...
    sub = OutcomeSubmission(
        user_id=1, exam_type="JEE_MAIN", counseling_body="JoSAA", year=2026, round_number=1,
        college_code="IIT_K", branch_code="EE", category="GENERAL", quota="OS", student_rank=1500,
        data_confidence="LOW", is_anomalous=True
    )
    db.add(sub)
    item = SMEReviewQueue(
        exam_type="JEE_MAIN", counseling_body="JoSAA", year=2026, round_number=1,
        college_code="IIT_K", branch_code="EE", category="GENERAL", quota="OS",
        opening_rank=1500, closing_rank=1500, source_url="Test", reason="Test",
        resolved=False
    )
    db.add(item)

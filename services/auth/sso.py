...
async def verify_google_token(token: str) -> dict | None:
    if settings.ENVIRONMENT == "development" and not settings.GOOGLE_CLIENT_ID:
        ...
        return {"sub": "google_test_123", "email": "google_user@example.com", "name": "Google User"}
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if resp.status_code != 200:
                log_sso_error("google", "Non-200 status code from Google tokeninfo", resp.text)
                return None
            data = resp.json()
            # Verify client ID if configured
            if settings.GOOGLE_CLIENT_ID and data.get("aud") != settings.GOOGLE_CLIENT_ID:
                log_sso_error("google", "Client ID mismatch", f"Audience was {data.get('aud')}")
                return None
            log_sso_success("google")
            return data
    except Exception as e:
        log_sso_error("google", "HTTP request exception during Google verification", str(e))
        return None

async def verify_apple_token(token: str) -> dict | None:
    if settings.ENVIRONMENT == "development" and not settings.APPLE_CLIENT_ID:
        # Mock payload for testing
        return {"sub": "apple_test_123", "email": "apple_user@example.com", "name": "Apple User"}

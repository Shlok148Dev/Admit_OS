...
from services.career.db import (
    Base,
    engine,
    get_db,
    get_neo4j_session,
...
def get_paths(req: CareerPathsRequest) -> CareerPathsResponse:
    try:
        with get_neo4j_session() as session:
            ...
            if not paths:
                raise HTTPException(
                    status_code=404, detail=f"Branch code {req.branch_code} not found"
                )
...
def get_branch(code: str) -> BranchOverviewResponse:
    try:
        with get_neo4j_session() as session:
            ...
            if not overview:
                raise HTTPException(
                    status_code=404, detail=f"Branch code {code} not found"
                )
            return overview
    except HTTPException:
        raise
    ...
    try:
        with get_neo4j_session() as session:
            ...
            if not overview1 or not overview2:
                raise HTTPException(
                    status_code=404, detail="One or both branch codes not found"
                )

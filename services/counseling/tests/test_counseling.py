...
from services.counseling.optimizer import (
    filter_colleges_by_exam,
    CandidateCollege,
    ChoiceOutput,
    sort_by_risk_appetite,
    get_choice_label,
    apply_upgrade_optimization,
)
...
def test_what_if_simulation():
    ...
    orig_prob = [
        ...
        for c in data["original_choices"]
        if c["college_code"] == "IIT_BOMBAY"
    ][0]
    mod_prob = [
        c["admission_probability"]
        for c in data["modified_choices"]
        if c["college_code"] == "IIT_BOMBAY"
    ][0]
...
def test_chat_rag():
    ...
    try:
        resp = client.post(
            ...
            json={
                "session_id": "test",
                "query": "What is the float option in JoSAA counseling?",
            },
        )
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert (
            "degraded" in data["answer"].lower()
            or "degraded" in (data.get("warning") or "").lower()
        )

        # Fallback response for genuinely irrelevant query
        resp2 = client.post(
            "/v1/counsel/chat",
            json={"session_id": "test", "query": "xyzzy quux flurble"},
        )
...
def test_degraded_mode_toggle_and_chat_bypass():
    ...
    resp_toggle_on = client.post(
        "/v1/admin/degraded-mode?enabled=true", auth=("admin", "admin_secure_pass123")
    )
    ...
    resp_chat = client.post(
        "/v1/counsel/chat", json={"session_id": "test", "query": "What is float?"}
    )
    ...
    resp_toggle_off = client.post(
        "/v1/admin/degraded-mode?enabled=false", auth=("admin", "admin_secure_pass123")
    )

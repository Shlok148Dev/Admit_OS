...
from services.counseling.rag.chat import _detect_query_style, _build_system_prompt, ARIAChatEngine
...
def test_query_style_detection():
    ...
    assert _detect_query_style("What is the cutoff for CSE at NIT Trichy?") == "CUTOFF_CHANCES"
    assert _detect_query_style("Can you explain the float vs freeze rules?") == "RULES_QA"
    assert _detect_query_style("Which is better between IIT Bombay and IIT Delhi?") == "COMPARISON"
...
def test_config_loading_and_fallback():
    ...
    with patch.dict(os.environ, {"REDIS_HOST": "test-redis-host", "ANTHROPIC_API_KEY": "test-key"}):
        settings = Settings()
...
def test_redis_history_chat_flow(mock_redis):
    ...
    with patch("services.counseling.main._chat_engine.chat") as mock_chat:
        ...
        req_body = {
            ...
            "history": [],
            "exam_type": "JEE_MAIN",
            "student_context": {"rank": 5000, "category": "GENERAL", "home_state": "MH"}
        }

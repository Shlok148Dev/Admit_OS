...
COLLEGES_DATA: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "state": "DL",
        "city": "New Delhi",
        "nirf_rank_engineering": 2,
        "established_year": 1961,
    },
    {
        "college_code": "IIT_MADRAS",
        "name": "IIT Madras",
        "type": "IIT",
        "state": "TN",
        "city": "Chennai",
        "nirf_rank_engineering": 1,
    ...
    },
    {
        ...
        "state": "UP",
        "city": "Allahabad",
        "nirf_rank_engineering": 25,
        "established_year": 1999,
    },
    {
        "college_code": "IIIT_DELHI",
        "name": "IIIT Delhi",
        "type": "IIIT",
        "state": "DL",
        "city": "New Delhi",
        "nirf_rank_engineering": 35,
    ...
    },
    {
        ...
        "state": "MH",
        "city": "Pune",
        "nirf_rank_engineering": 73,
        "established_year": 1854,
    },
    {
        "college_code": "VJTI_MUMBAI",
        "name": "VJTI Mumbai",
        "type": "STATE",
        "state": "MH",
        "city": "Mumbai",
        "nirf_rank_engineering": 82,
        "established_year": 1887,
    },
    {
        "college_code": "ICT_MUMBAI",
        "name": "ICT Mumbai",
        "type": "STATE",
        "state": "MH",
        "city": "Mumbai",
        "nirf_rank_engineering": 95,
...
def populate_synthetic_data(db: Session) -> None:
    ...
    for c in COLLEGES_DATA:
        if (
            not db.query(College)
            .filter(College.college_code == c["college_code"])
            .first()
        ):
            db.add(College(**c))
    ...
    for _, row in df_m.iterrows():
        cbody = (
            "JOSAA"
            if row["college_code"].startswith(("IIT", "NIT", "IIIT"))
            else "DTE_MH"
        )
        db.add(
            ExamCutoff(
                exam_type="JEE_MAIN",
                counseling_body=cbody,
                year=row["year"],
                round_number=6,
    ...
    neet_colleges = [
        {
            ...
            "state": "DL",
            "city": "New Delhi",
            "nirf_rank_engineering": None,
            "established_year": 1956,
        },
        {
            "college_code": "MAMC_DELHI",
            "name": "MAMC Delhi",
            "type": "STATE",
            "state": "DL",
            "city": "New Delhi",
...
def load_production_models() -> None:
    ...
    for exam in ["JEE_MAIN", "NEET", "MHT_CET"]:
        ...
        try:
            ...
            elif hasattr(loaded, "_model_impl") and hasattr(
                loaded._model_impl, "python_model"
            ):
...
def run_initial_training_in_background() -> None:
    """Run model training and mlflow lifecycle in a background thread."""
    try:
        logger.info("Starting initial model training/lifecycle in background thread...")
        from .training.train_per_exam import main as run_training
        from .training.mlflow_lifecycle import main as run_lifecycle
        run_training()
        run_lifecycle()
        load_production_models()
        logger.info("Background initial training/lifecycle completed successfully.")
    except Exception as e:
        logger.error(f"Failed background model training: {e}")
...
def startup_event() -> None:
    ...
    try:
        ...
        load_production_models()
        if not predictors:
            logger.info("No production models. Spawning initial training/lifecycle in background...")
            import threading
            threading.Thread(target=run_initial_training_in_background, daemon=True).start()
...
def generate_cache_key(request: CollegePredictionRequest) -> str:
    ...
    branches = (
        request.filters.branches
        if (request.filters and request.filters.branches)
        else []
    )
    sorted_branches = sorted(branches)
    branch_filter_hash = hashlib.sha256(
        ",".join(sorted_branches).encode("utf-8")
    ).hexdigest()[:12]
...
def get_college_fees(college_code: str) -> int:
    ...
    fees = {
        ...
        "IIT_MADRAS": 210000,
        "NIT_TRICHY": 147150,
        "NIT_SURATHKAL": 150000,
        "IIIT_ALLAHABAD": 180000,
        "IIIT_DELHI": 350000,
        "COEP_PUNE": 135000,
        "VJTI_MUMBAI": 85000,
        "ICT_MUMBAI": 90000,
...
def get_db_cutoff(
    ...
    if cache is not None:
        key = (col_code, br_code, cat, quota)
        if key in cache and year in cache[key]:
            return float(cache[key][year][1])
        return None
    res = (
        db.query(ExamCutoff)
        .filter(
            ExamCutoff.college_code == col_code,
            ExamCutoff.branch_code == br_code,
            ExamCutoff.category == cat,
            ExamCutoff.quota == quota,
            ExamCutoff.year == year,
...
def get_historical_ranks(
    ...
    if cache is not None:
        key = (col_code, br_code, cat, quota)
        if key in cache:
            return {str(y): val[1] for y, val in sorted(cache[key].items())}
        return {}
    res = (
        db.query(ExamCutoff)
        .filter(
...
def matches_filters(
    ...
    if request.exam == ExamEnum.NEET:
        if col.type not in ("DEEMED", "MEDICAL") and col.college_code not in (
            "AIIMS_DELHI",
            "MAMC_DELHI",
        ):
            return False
...
def get_lags_for_prediction(
    ...
    lag_1 = get_db_cutoff(db, college, branch, category, quota, 2024, exam_type, cache)
    lag_2 = get_db_cutoff(db, college, branch, category, quota, 2023, exam_type, cache)
    if exam_type == "NEET":
        if lag_1 is None:
            return 0.0, 0.0
        if lag_2 is None:
...
def generate_single_prediction(
    col: College, branch_code: str, request: CollegePredictionRequest, db: Session, cache: Optional[Dict] = None
    ...
    if request.exam == ExamEnum.NEET:
        ...
        quota = "MS" if col.state == request.home_state else "AI"
    else:
        # JEE_MAIN (JoSAA/DTE-state/JAC-Delhi)
        gov_body = getattr(col, "governing_body", "JOSAA")
        if gov_body == "DTE-state":
            quota = "MS" if col.state == request.home_state else "AI"
        elif gov_body == "JAC_DELHI":
            quota = "HS" if col.state == request.home_state else "OS"
        else:
            # JOSAA default (HS / OS)
            quota = (
                "HS"
                if col.state == request.home_state and col.type in ("NIT", "STATE")
                else "OS"
            )
    ...
    hist_ranks = get_historical_ranks(
        db, col.college_code, branch_code, request.category.value, quota, exam_type, cache
    )
    if not hist_ranks:
        return None
    ...
    if len(hist_ranks) == 1:
        # Use actual database records to prevent model artifacts
        single_year = int(list(hist_ranks.keys())[0])
        actual_op = int(lag_1 * 0.95)
        actual_cl = int(lag_1)
        if cache is not None:
            key = (col.college_code, branch_code, request.category.value, quota)
            if key in cache and single_year in cache[key]:
                actual_op = int(cache[key][single_year][0])
                actual_cl = int(cache[key][single_year][1])
        p10 = actual_op
        p50 = actual_cl
        p90 = int(actual_cl * 1.05)
        prob = 1.0 if request.rank <= p50 else (0.0 if request.rank > p50 * 1.1 else (p50 * 1.1 - request.rank) / (p50 * 0.1))
    else:
    ...
    # Retrieve actual source_url from database/cache
    actual_source_url = None
    if cache is not None:
        key = (col.college_code, branch_code, request.category.value, quota)
        if key in cache and 2024 in cache[key]:
            if len(cache[key][2024]) > 3:
                actual_source_url = cache[key][2024][3]
    else:
        latest_cutoff = (
            db.query(ExamCutoff)
            .filter(
                ExamCutoff.college_code == col.college_code,
                ExamCutoff.branch_code == branch_code,
                ExamCutoff.category == request.category.value,
                ExamCutoff.quota == quota,
                ExamCutoff.exam_type == exam_type,
        ...
        if latest_cutoff:
            actual_source_url = latest_cutoff.source_url
    ...
    response_quota = quota
    if request.exam == ExamEnum.JEE_MAIN and getattr(col, "governing_body", "") == "DTE-state":
        if quota == "OS":
            response_quota = "AI"
        elif quota == "HS":
            response_quota = "MS"

    # Determine confidence based on database records and historical depth (number of years)
    num_years = len(hist_ranks)
    if num_years >= 3:
    ...
    # Also check if any database record has data_confidence == "LOW" or "MEDIUM" to demote if necessary
    if cache is not None:
        key = (col.college_code, branch_code, request.category.value, quota)
        if key in cache:
            for val in cache[key].values():
                if len(val) >= 3:
                    conf = val[2]
                    if conf == "LOW":
                        calculated_confidence = "LOW"
                    elif conf == "MEDIUM" and calculated_confidence == "HIGH":
                        calculated_confidence = "MEDIUM"
    else:
        db_cutoffs = (
            db.query(ExamCutoff)
        ...
        if any(c.data_confidence == "LOW" for c in db_cutoffs):
            calculated_confidence = "LOW"
        elif any(c.data_confidence == "MEDIUM" for c in db_cutoffs) and calculated_confidence == "HIGH":
            calculated_confidence = "MEDIUM"
    ...
    return CollegePrediction(
        ...
        branch_name=(
            f"{br_name} Engineering" if branch_code in ("CS", "EC", "ME") else br_name
        ),
        quota=response_quota,
        predicted_opening_rank=int(p50 * 0.85),
        predicted_closing_rank=p50,
        confidence_interval={"p10": p10, "p50": p50, "p90": p90},
        admission_probability=prob,
        historical_closing_ranks=hist_ranks,
        trend=compute_trend(hist_ranks),
        data_confidence=calculated_confidence,
        data_source=f"Official Cutoff {col.type}",
        source_url=source_url,
        fees_per_year=get_college_fees(col.college_code),
        nirf_rank=col.nirf_rank_engineering,
    )
...
def run_prediction_pipeline(
    ...
    # Pre-fetch all cutoffs for this category and exam type to prevent N+1 query overload
    target_categories = [request.category.value]
    if request.exam.value == "MHT_CET" and request.category.value == "GENERAL":
        target_categories.extend(["GOPENH", "GOPENS", "GOPENO"])

    all_cutoffs = (
        db.query(ExamCutoff)
        .filter(
            ExamCutoff.exam_type == request.exam.value,
            ExamCutoff.category.in_(target_categories),
        )
        .all()
    )

    
    # Build cache: (college, branch, category, quota) -> year -> (opening, closing, data_confidence, source_url)
    cache = {}
    for c in all_cutoffs:
        key = (c.college_code, c.branch_code, c.category, c.quota)
        if key not in cache:
            cache[key] = {}
        cache[key][c.year] = (c.opening_rank, c.closing_rank, c.data_confidence, c.source_url)

    for col in colleges:
        for branch_code in branches:
            if not matches_filters(col, branch_code, request):
                continue
            pred = generate_single_prediction(col, branch_code, request, db, cache)
            if pred:
...
def predict_college(
    ...
    if exam_type not in predictors:
        load_production_models()
    # Gracefully fall back to CutoffPredictor when no production model is in registry (dev/test environments only)
    if exam_type not in predictors:
        env = os.getenv("ENVIRONMENT", "").strip().lower()
        if env in ("development", "test"):
            logger.warning(
                f"No production model for {exam_type}. Using fallback CutoffPredictor in dev/test mode."
            )
            predictors[exam_type] = CutoffPredictor()
        else:
            logger.error(
                f"Production model for {exam_type} is unavailable. Refusing synthetic fallback in production environment: '{env}'."
            )
            raise HTTPException(
                status_code=503,
                detail=f"Prediction service is unavailable: ML models for {exam_type} are offline."
            )
        ...
        with _uncached_lock:
            if _current_uncached_queries >= MAX_CONCURRENT_UNCACHED:
                ...
                if wrapped and "wrapped_response" in wrapped:
                    logger.warning(
                        "Serving stale cached result under high concurrency limit."
                    )
                    req.state.is_cached = True
                    return CollegePredictionResponse(**wrapped["wrapped_response"])
                raise HTTPException(
    ...
    try:
        ...
        if wrapped and "wrapped_response" in wrapped:
            logger.warning(
                "DB/pipeline query failed. Serving stale cached result as fallback."
            )
            req.state.is_cached = True
            return CollegePredictionResponse(**wrapped["wrapped_response"])
        raise HTTPException(
...
def get_college_profile(
    code: str, req: Request, db: Session = Depends(get_db)
) -> CollegeProfileSchema:
...
def detailed_health() -> Dict[str, Any]:
    ...
    try:
        ...
        active_connections = (
            ...
            if hasattr(engine, "pool") and hasattr(engine.pool, "checkedout")
            else -1
        )
        pool_size = (
            engine.pool.size()
        ...
        max_overflow = (
            ...
            if hasattr(engine, "pool") and hasattr(engine.pool, "overflow")
            else -1
        )
    except Exception as ex:
    ...
    return {
        ...
        "redis": {"latency_ms": redis_lat, "healthy": redis_lat >= 0},
        "hit_rate": hit_rate,

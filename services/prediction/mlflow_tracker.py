            ...
            for name, model in models.items():
                ...
                if "xgb" in name.lower():
                    mlflow.xgboost.log_model(
                        model, artifact_path=name, registered_model_name=model_name
                    )
                ...
                try:
                    ...
                    if latest_versions:
                        ...
                        client.set_model_version_tag(
                            model_name, latest_ver, "exam_type", exam_type
                        )
                        logger.info(
                            f"Registered model {model_name} version {latest_ver} tagged with exam_type: {exam_type}"
                        )
                except Exception as reg_err:
                    logger.warning(
                        f"Could not set registry tag for {model_name}: {reg_err}"
                    )

...
class PredictionFilters(BaseModel):
    ...
    college_types: Optional[List[str]] = Field(None, description="Preferred college types")
...
class CollegePredictionRequest(BaseModel):
    ...
    percentile: Optional[float] = Field(None, ge=0.0, le=100.0, description="Student's percentile")
    category: CategoryEnum
    home_state: str = Field(..., min_length=2, max_length=2, description="ISO state code")
    gender: GenderEnum
    year: int = Field(..., description="Counseling year")

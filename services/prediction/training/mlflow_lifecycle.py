...
def get_latest_candidate_version(
    ...
    try:
        ...
        logger.warning(
            f"No versions found or error retrieving model versions for {model_name}: {e}"
        )
    return None
...
def get_shadow_logs(db: Session, exam_type: str) -> List[PredictionLog]:
    ...
    logger.info(
        f"Fewer than 5 logs for {exam_type}. Generating mock logs from 2024 cutoffs."
    )
    cutoffs = (
        db.query(ExamCutoff)
        .filter(ExamCutoff.exam_type == exam_type, ExamCutoff.year == 2024)
        .limit(50)
        .all()
    )
    ...
    for c in cutoffs:
        mock_logs.append(
            PredictionLog(
                ...
                gender="M",
                rank=c.closing_rank,
                predicted_closing_rank=c.closing_rank,
...
def evaluate_on_logs(
    db: Session, logs: List[PredictionLog], model: Any, exam_type: str
) -> float:
    ...
    for log in logs:
        ...
        actual = get_actual_cutoff(
            db, exam_type, log.college_code, log.branch_code, log.category, log.quota
        )
        if actual is None:
            actual = log.rank
        try:
...
def process_exam_promotion(db: Session, client: MlflowClient, exam_type: str) -> None:
    ...
    logger.info(
        f"{model_name} v{ver.version} shadow test accuracy: {accuracy:.4f} (Required: {threshold})"
    )

    if accuracy >= threshold:
        promote_model_version(client, model_name, ver.version)
        logger.info(
            f"Successfully promoted {model_name} version {ver.version} to Production."
        )
    else:
        logger.warning(
            f"{model_name} version {ver.version} did not meet gating criteria."
        )
